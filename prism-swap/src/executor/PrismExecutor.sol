...
import "../interface/IPrismExecutor.sol";
 ...
 */
contract PrismExecutor is IExecutor, Ownable, ReentrancyGuard {
    ...
    function executeSplitSwap(IERC20 fromToken, IERC20 toToken, PrismStructs.RouteGroup[] calldata paths)
        external
        payable
        override
        onlyOwner
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < paths.length; i++) {
            ...
            executeHopPath(address(fromToken), amount, paths[i].paths);
        }
        require(totalPercent == SignedDecimalMath.ONE, "PrismExecutor: route percent != 1e18");
        uint256 balance = toToken.universalBalanceOf(address(this));
        toToken.universalTransfer(payable(msg.sender), balance);
    }

    function executeHopPath(address fromToken, uint256 fromTokenAmount, PrismStructs.HopPath[] calldata paths) internal {
        for (uint256 i = 0; i < paths.length; i++) {
            ...
            for (uint256 j = 0; j < paths[i].adapters.length; j++) {
                PrismStructs.Adapter memory adapter = paths[i].adapters[j];
                require(approvedAdapters[adapter.adapter], "PrismExecutor: adapter not approved");
            ...
            require(totalPercent == SignedDecimalMath.ONE, "PrismExecutor: adapter percent != 1e18");
    ...
    function setDEXAdapter(address _adapter, bool isAdd) external onlyOwner {
        approvedAdapters[_adapter] = isAdd;
        if (isAdd) {

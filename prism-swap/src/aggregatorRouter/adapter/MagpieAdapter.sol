...
contract MagpieAdapter is ReentrancyGuard {
    address public immutable magpieApprover;
    ...
    function swapOnAdapter(
        ...
        if (fromToken != UniversalERC20.ETH) {
            // approve
            IERC20(fromToken).forceApprove(magpieApprover, fromTokenAmount);
            magpieRouter.functionCallWithValue(data, 0);
        } else {
            // For ETH, use fromTokenAmount instead of msg.value to handle fee deduction
            magpieRouter.functionCallWithValue(data, fromTokenAmount);
        }

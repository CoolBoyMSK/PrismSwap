...
contract ParaswapAdapter is ReentrancyGuard {
    address public immutable paraswapApprover;
    address public immutable paraswapRouter;
    ...
    function swapOnAdapter(
        ...
        if (fromToken != UniversalERC20.ETH) {
            // approve
            IERC20(fromToken).forceApprove(paraswapApprover, fromTokenAmount);
            paraswapRouter.functionCallWithValue(data, 0);
        } else {
            // For ETH, use fromTokenAmount instead of msg.value to handle fee deduction
            paraswapRouter.functionCallWithValue(data, fromTokenAmount);
        }

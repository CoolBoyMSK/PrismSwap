...
contract OneinchAdapter is ReentrancyGuard {
    address public immutable oneInchApprover;
    ...
    function swapOnAdapter(
        ...
        if (fromToken != UniversalERC20.ETH) {
            // approve
            IERC20(fromToken).forceApprove(oneInchApprover, fromTokenAmount);
            oneInchRouter.functionCallWithValue(data, 0);
        } else {
            // For ETH, use fromTokenAmount instead of msg.value to handle fee deduction
            oneInchRouter.functionCallWithValue(data, fromTokenAmount);
        }

        // transfer remaining fromTokens back to recipient
        ...
        // transfer all received toTokens back to caller (PrismProxy -> PrismHub)

...
contract PrismProxy {
    ...
    function swap(address adapter, bytes calldata data) external payable {
        if (msg.sender != hubContract) revert PrismErrors.Forbidden();
        if (adapter == address(0)) revert PrismErrors.AdapterNotApproved();

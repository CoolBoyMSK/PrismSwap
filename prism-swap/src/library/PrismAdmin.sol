...
abstract contract PrismAdmin {
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == admins[0] || msg.sender == admins[1] || msg.sender == admins[2], "PrismAdmin: not admin"
        );
        _;
    }

    modifier whenNotPaused() {
        require(!paused, "PrismAdmin: paused");
        _;
    }
    ...
    function unpause() external onlyAdmin {
        require(pauseCount < 2, "PrismAdmin: multiple admins paused");
        require(pauseCount > 0, "PrismAdmin: not paused");
        if (adminPaused[msg.sender]) {
    ...
    function updateAdmins(address newAdmin) external onlyAdmin {
        require(newAdmin != address(0), "PrismAdmin: zero address");
        require(
            newAdmin != admins[0] && newAdmin != admins[1] && newAdmin != admins[2], "PrismAdmin: already an admin"
        );
        require(!adminPaused[msg.sender], "PrismAdmin: unpause first");

...
import "../executor/PrismExecutor.sol";
 ...
 */
contract PrismRouter is PrismAdmin, ReentrancyGuard {
    ...
    event DEXAdapterUpdated(address indexed adapter, bool add);
    ...
    function swap(
        ...
        uint256 feeRate,
        address feeReceiver,
        PrismStructs.RouteGroup[] calldata paths
    ) external payable whenNotPaused nonReentrant {
        require(fromToken != toToken, "PrismRouter: fromToken == toToken");
        require(feeRate <= maxFeeRate, "PrismRouter: feeRate exceeds max");
        require(msg.value == (fromToken == UniversalERC20.ETH ? fromTokenAmount : 0), "PrismRouter: incorrect msg.value");
        uint256 feeAmount;
        ...
        executor.executeSplitSwap{value: fromToken == UniversalERC20.ETH ? fromTokenAmount : 0}(
        ...
        // check slippage
        require(receivedAmount >= minAmountOut, "PrismRouter: slippage limit exceeded");
    ...
    function setDEXAdapter(address _adapter, bool isAdd) external onlyAdmin {
        executor.setDEXAdapter(_adapter, isAdd);
        emit DEXAdapterUpdated(_adapter, isAdd);
    }
